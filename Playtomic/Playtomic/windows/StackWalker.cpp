...
void StackWalker::GetCallStackString(std::string& dest)
{
	...
	it = m_stackList.begin();

	if( m_stackList.size() > 10)
	{
		advance(it, 10);
	}
	else
	{
		m_stackList.push_front(std::string("stack to short"));
	}
	

	for(; it != m_stackList.end(); it++)
	{
		dest += *it;
	}
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    //OnOutput(buffer);
	m_stackList.push_back(std::string(buffer));
	//m_StackData += buffer;
  }
}
